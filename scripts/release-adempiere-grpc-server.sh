...
#
#  Assumes: commits have already been pushed to feature/shw/master.
#
#  Usage:
#    ./release-adempiere-grpc-server.sh "<release notes>"
#    ./release-adempiere-grpc-server.sh --dry-run "<release notes>"
#    ./release-adempiere-grpc-server.sh -n
# =============================================================================
...
if ! $DRY_RUN && [ -z "$RELEASE_NOTES" ]; then
    ...
    echo "  Assumes: commits are already pushed to feature/shw/master."
    echo ""
    echo "  Usage:"
...
GRPC_REPO="Systemhaus-Westfalia/adempiere-grpc-server"
GRPC_BRANCH="feature/shw/master"
...
# ── Version helpers ────────────────────────────────────────────────────────────
# Increment the last numeric component: 3.9.4.001-shw-1.0.42 → 3.9.4.001-shw-1.0.43
increment_patch() {
    local v="$1"
    local prefix last
...
# ─────────────────────────────────────────────────────────────────────────────
step_start "Step 1/5  adempiere-grpc-server — wait for CI"
...
step_start "Step 2/5  adempiere-grpc-server — create release"
...
step_start "Step 3/5  adempiere-grpc-server — wait for publish"
...
step_start "Step 4/5  adempiere-grpc-server — verify Docker image"
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 2 — adempiere-ui-gateway: update grpc-server version, commit, push
# ─────────────────────────────────────────────────────────────────────────────
step_start "Step 5/5  adempiere-ui-gateway — update grpc-server version"
...
echo "      VUE_BACKEND_GRPC_SERVER_VERSION → ${GRPC_SHORT_NEW}  (full image: ${GRPC_NEW})"
