	...
	private boolean explodeBOM()
	{
		...
		while (count != 0)
		{
			...
			for (int i = 0; i < lines.length; i++)
			{
				...
				X_PP_Product_BOM bom = getProductBom(product, getAD_Org_ID(), getDatePromised(), get_TrxName());
				if(bom != null) {
					getProductBomLines(bom.getPP_Product_BOM_ID()).forEach(productBomLineId -> {
						...
						//	Validate
						if (TimeUtil.isValid(productBomLine.getValidFrom(), productBomLine.getValidTo(), getDatePromised())) {
	...
	private MInvoice createInvoice (MDocType dt, MInOut shipment, Timestamp invoiceDate)
	{
		...
		if (issplit) {	
			...
			for (MOrderLine line : oLines) {
				if(line.getC_Charge_ID() > 0 && line.getC_Charge().getC_ChargeType().getValue().equals("CTAJ"))
					count++;
			}
			if (count==0)
				issplit = false;				
		...
		if (issplit) {			
			//SHW SA
			notaDebito = new MInvoice(this, dt.getC_DocTypeInvoice().getC_DocTypeDifference_ID(), invoiceDate);
			notaDebito.saveEx();
		}			
		...
		if (shipment != null)
		{
			...
			for (int i = 0; i < sLines.length; i++)
			{
				...
				{
					//SHW SA
					if (issplit && sLine.getC_Charge_ID() > 0 && sLine.getC_Charge().getC_ChargeType().getValue().equals( "CTAJ"))
					{
						iLine = new MInvoiceLine(notaDebito);
					}
					else
		...
		else	//	Create Invoice from Order
		{
			...
			for (int i = 0; i < oLines.length; i++)
			{
				MOrderLine oLine = oLines[i];
				//SHW SA
				if (issplit && oLine.getC_Charge_ID() > 0 && oLine.getC_Charge().getC_ChargeType().getValue()== "CTAJ")
				{
					iLine = new MInvoiceLine(notaDebito);
				}
				else
		...
		if (notaDebito != null)
		{
			String docAction = dt.getC_DocTypeInvoice().isPrepareSplitDocument()? DOCACTION_Prepare: DOCACTION_Complete;
			notaDebito.processIt(docAction);
			notaDebito.saveEx(get_TrxName());
			if (invoice.getLines().length == 0)
				return notaDebito;
		}
