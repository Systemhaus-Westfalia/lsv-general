	...
	public String docValidate (PO po, int timing)
	{
		...
		if (po.get_TableName().equals(MInvoice.Table_Name)
			...
			if (inv.getDescription() != null 
				...
				if (inv.get_Value("WithholdingAmt") == null || WithholdingAmt.compareTo(Env.ZERO)==0) {
		...
		if (po.get_TableName().equals(MInvoice.Table_Name) && timing == TIMING_AFTER_PREPARE) {
...
//		}
	...
	private String validateWriteOffVsPaymentWithholdings(MPayment pay) {
		if (pay.getC_Invoice_ID() > 0) {
			...
			MCurrency currency = (MCurrency)pay.getC_Currency();
			List<MLCOInvoiceWithholding> iwhs = new Query(pay.getCtx(), MLCOInvoiceWithholding.Table_Name, whereClause, pay.get_TrxName())
				.setParameters(pay.getC_Invoice_ID())
			...
			for (MLCOInvoiceWithholding iwh:iwhs)
			{
				...
				{
					CurrencyRate = MConversionRate.getRate (iwh.getC_Invoice().getC_Currency_ID(),pay.getC_Currency_ID(),
							 pay.getDateAcct(), pay.getC_ConversionType_ID(), pay.getAD_Client_ID(),
							0);
				}					
				sumwhamt = sumwhamt.add(CurrencyRate.multiply(iwh.getTaxAmt()).setScale(currency.getStdPrecision(), BigDecimal.ROUND_HALF_UP));
			}/*
			...
			try {
				...
				while (rs.next()) {
					...
					for (MLCOInvoiceWithholding iwh:iwhs)
					{
						BigDecimal CurrencyRate = Env.ONE;
						if (iwh.getC_Invoice().getC_Currency_ID() != pay.getC_Currency_ID())
							CurrencyRate = MConversionRate.getRate (iwh.getC_Invoice().getC_Currency_ID(),
									pay.getC_Currency_ID(), pay.getDateAcct(), pay.getC_ConversionType_ID(), pay.getAD_Client_ID(),
									0);
			...
			for (int j = 0; j < alloc_lines.length; j++) {
				...
				try
				{
					...
					while (rs.next()) {
						...
						if (alloc_line.getC_Invoice().getC_Currency_ID() != as.getC_Currency_ID())
							CurrencyRate = MConversionRate.getRate (alloc_line.getC_Invoice().getC_Currency_ID(),
									as.getC_Currency_ID(), alloc_line.getC_Payment().getDateAcct(), 
									alloc_line.getC_Payment().getC_ConversionType_ID(), alloc_line.getC_Payment().getAD_Client_ID(),
									0); 
				...
				if (Env.ZERO.compareTo(tottax) != 0)
				{
					...
					for (int ifl = 0; ifl < factlines.length; ifl++) {
						...
						// SHW: only fact lines from allocation line
						if (fl.getLine_ID()!=alloc_line.getC_AllocationLine_ID())
							continue;
