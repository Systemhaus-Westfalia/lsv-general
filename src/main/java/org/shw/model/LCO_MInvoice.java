...
public class LCO_MInvoice extends MInvoice
{
	...
	public int recalcWithholdings() {
		...
		try
		{
			...
			pstmtt.setInt(2, Env.getAD_Client_ID(getCtx()));
			...
			while (rst.next())
			{
				...
				while (rsr.next())
				{
					...
					if (wc.getBaseType() == null) {
						...
						base = getTotalLines();
						basenet = new Query(getCtx(), MInvoiceLine.Table_Name, " c_invoice_id=?", get_TrxName())
						.setParameters(getC_Invoice_ID())
						.aggregate(MInvoiceLine.COLUMNNAME_LineNetAmt, Query.AGGREGATE_SUM);
						...
						if (wrc.isUseWithholdingCategory() && wrc.isUseProductTaxCategory()) {
							...
							sqllca = 
								"SELECT SUM (LineNetAmt), sum(il.priceList*il.qtyinvoiced) "
								+ "  FROM C_InvoiceLine il "
						...
						if (wc.getC_BaseTax_ID() != 0) {
							...
							String sqlbst = "SELECT SUM(taxbaseamt) "
								+ " FROM C_InvoiceTax "
								+ " WHERE IsActive='Y' AND C_Invoice_ID = ? "
								+ "   AND C_Tax_ID = ?";
					...
					// SHW: consider different currency
					
					BigDecimal threshholdMin = BigDecimal.ZERO;
					BigDecimal threshholdMax = BigDecimal.ZERO;					

					final String whereClause = MInvoice.COLUMNNAME_AD_Client_ID+"=?";
					MAcctSchema as = new Query(getCtx(), I_C_AcctSchema.Table_Name, whereClause, get_TrxName())
							.setParameters(getAD_Client_ID()).first();
					
					if (getC_Currency_ID() == as.getC_Currency_ID()) {
						// same currency in account schema and invoice
						threshholdMin = wc.getThresholdmin();
						threshholdMax = wc.getThresholdMax();
					} 
					else {
						// different currency in account schema and invoice: calculate
						threshholdMin = MConversionRate.convert(getCtx(), wc.getThresholdmin(), as.getC_Currency_ID(), 
								getC_Currency_ID(), getDateAcct(), getC_ConversionType_ID(), getAD_Client_ID(),0);
						threshholdMax = MConversionRate.convert(getCtx(), wc.getThresholdMax(), as.getC_Currency_ID(), 
								getC_Currency_ID(), getDateAcct(), getC_ConversionType_ID(), getAD_Client_ID(),0);
					}
					...
					/*if (base != null && 
						...
						if (base != null && 
						base.compareTo(Env.ZERO) != 0 && 
						base.compareTo(threshholdMin) >= 0 &&
						(threshholdMax == null || threshholdMax.compareTo(Env.ZERO) == 0 || base.compareTo(threshholdMax) <= 0) &&
						tax.getRate() != null &&
						tax.getRate().compareTo(Env.ZERO) != 0) {
						...
						if (basenet != null && basenet.compareTo(Env.ZERO)!= 0 )
						{
							...
							iwh.set_ValueOfColumn("NetAmtToInvoice", basenet);	
							iwh.set_ValueOfColumn("taxnetamt", taxnetamt);						
						}
						iwh.save();
