package org.shw.lsv.ebanking.bac.sv.util;
...
public final class RequestParamsFactory {
     ...
     */
    public static RequestParams createCamt052Params(MBankAccount bankAccount) {

        String currentTimestamp    = TestDateUtils.getCurrentApiTimestamp();
        MBank bank = (MBank)bankAccount.getC_Bank();
        MClient client = new MClient(Env.getCtx(), bank.getAD_Client_ID(), null);
        ...
        String SALDO_MESSAGE_ID    = "Saldo-" + client.getName() + "/" + bankAccount.getAccountNo();
        ...
        String REQDMSGNMID         = bank.get_ValueAsString("REQDMSGNMID"); //"AccountBalanceReportV08";
        String CURRENCY            = bankAccount.getC_Currency().getISO_Code();

        String BAC_ACCOUNT_ID      = bankAccount.getAccountNo(); //"200268472"; // Bank Account ID
     ...
     */
    public static RequestParams createPain001Params_Domestico(MPayment payment) {
        ...
        String BIC_TO_EL_SALVADOR    = payment.getC_BP_BankAccount().getC_Bank().getSwiftCode();
        String PYMT_DOCUMENT_ID      = "PYMT-DOMESTICO-" + TestDateUtils.getTodayDate() + "-" + payment.getC_Payment_ID();// Generate a unique ID for testing purposes by appending a random 5-digit number.
     ...
     * @return A fully configured RequestParams object for a PAIN.001 test.
     */
    public static RequestParams createPain001Params_BAC_to_BAC(MPayment payment) {
    	MBankAccount bankAccount = (MBankAccount)payment.getC_BankAccount();
    	MBank bank = (MBank)bankAccount.getC_Bank();
        String BIC_SISTEMAS_AEREOS   = bank.get_ValueAsString("BIC_SISTEMAS_AEREOS");
        String BIC_BAC_EL_SALVADOR   = bank.getSwiftCode();
        String PYMT_DOCUMENT_ID      = "PYMT-BAC-TO-BAC-" + TestDateUtils.getTodayDate() + "-" + payment.getC_Payment_ID();// Generate a unique ID for testing purposes by appending a random 5-digit number.
                                        //java.util.concurrent.ThreadLocalRandom.current().nextInt(10000, 100000);
        String MSGDEFIDR             = "PaymentInitiationServiceV03";
        String BIZSVC                = bank.get_ValueAsString("BIZSVC");
        String CURRENCY              = payment.getC_BankAccount().getC_Currency().getISO_Code();
