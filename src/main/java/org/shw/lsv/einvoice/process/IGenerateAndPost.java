...
public interface  IGenerateAndPost {
	...
	public final static String whereClauseInOut = "AD_CLIENT_ID = ?  "
			+ " AND Exists (select 1 from c_Doctype dt where dt.c_Doctype_ID=m_Inout.c_Doctype_ID AND E_DocType_ID is not null) "
			+ " AND processed = 'Y' AND dateacct>=?  AND processing = 'N' "
			+ " AND ei_Processing = 'N' "
			+ " AND (docstatus IN ('CO','CL') OR coalesce(reversal_ID,0) > M_Inout_ID)"
			+ " AND (ei_Status_Extern is NULL OR ei_Status_Extern <> 'Firmado')";
	
	public final static String whereClauseInOutVoided = "AD_CLIENT_ID = ?  "
			+ " AND Exists (select 1 from c_Doctype dt where dt.c_Doctype_ID=m_InOut.c_Doctype_ID AND E_DocType_ID is not null) "
			+ " AND processed = 'Y' AND dateacct>=?  AND processing = 'N' "
			+ " AND ei_Processing = 'N' "
			+ " AND (docstatus IN ('RE','VO') AND reversal_ID is not null and reversal_ID<c_Invoice_ID)"
	...
	public static String getWhereclauseInOut(Boolean isVoided) {
		String result = isVoided? whereClauseInOutVoided:whereClauseInOut;
		return result;
	}
