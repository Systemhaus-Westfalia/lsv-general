 ...
 */
public class EInvoiceGenerateAndPostFindex extends EInvoiceGenerateAndPostFindexAbstract implements IGenerateAndPost
{
	...
	protected String processVoided() throws Exception{
		...
		System.out.println("\n" + "******************************************************");
		System.out.println("Process EInvoiceGenerateAndPost Voided: started with Client '" + client.getName() + "', ID: " + Env.getAD_Client_ID(getCtx()));
		...
		try {
			int[] invoiceIds = new Query(Env.getCtx(), MInvoice.Table_Name, whereClause, null)
						.setParameters(Env.getAD_Client_ID(getCtx()), startdate)
						.getIDs();
	...
	protected String processInvoices() throws Exception{
		...
		startdate = (Timestamp)(client.get_Value("ei_Startdate"));
		System.out.println("\n" + "******************************************************");
		System.out.println("Process EInvoiceGenerateAndPost: started with Client '" + client.getName() + "', ID: " + Env.getAD_Client_ID(getCtx()));
		registration = new Query(getCtx(), MADAppRegistration.Table_Name, "EXISTS(SELECT 1 FROM AD_AppSupport s "
				+ "WHERE s.AD_AppSupport_ID = AD_AppRegistration.AD_AppSupport_ID "
				+ "AND s.ApplicationType = ?"
				+ "AND s.IsActive = 'Y'"
