 ...
 *          C_AcctSchema.IsPostIfClearingEqual Avoid posting if Receipt and both
 *          accounts Unallocated Cash and Receivable are equal Avoid posting if
 *          Payment and both accounts Payment Select and Liability are equal
 * 
 * @author phib BF [ 2019262 ] Allocation posting currency gain/loss omits line
 *         reference
 * @author Yamel Senih, ysenih@erpya.com , http://www.erpya.com Add support to
...
public class Doc_AllocationHdr extends Doc {
	...
	/**************************************************************************
	 * Get Source Currency Balance - subtracts line and tax amounts from total - no
	 * rounding
	 * 
	 ...
	 * </pre>
	 * 
	 * Tax needs to be corrected for discount & write-off; Currency gain & loss is
	 * realized here.
	 * 
	...
	public ArrayList<Fact> createFacts(MAcctSchema as) {
		...
		Fact factForRGL = new Fact(this, as, Fact.POST_Actual); // dummy fact (not posted) to calculate Realized Gain &
																// Loss
		...
		for (int i = 0; i < p_lines.length; i++) {
			...
			if (line.getC_Payment_ID() != 0 && line.getC_Invoice_ID() == 0 && line.getC_Order_ID() == 0
					&& line.getC_CashLine_ID() == 0 && line.getC_BPartner_ID() == 0
					&& Env.ZERO.compareTo(line.getDiscountAmt()) == 0 && Env.ZERO.compareTo(line.getWriteOffAmt()) == 0)
				continue;
			...
			if (invoice == null) {
				/**
				 * Adaxa: Paul Allocate to charges
				 */
				if (line.getC_Invoice_ID() == 0 && line.getC_Payment_ID() == 0 && line.getC_Charge_ID() != 0) {
					// Adaxa
					fl = fact.createLine(line, line.getChargeAccount(as, line.getAmtSource()), getC_Currency_ID(),
							line.getAmtSource());

				} else if (line.getC_Invoice_ID() == 0 && line.getC_Charge_ID() != 0 && line.getC_Payment_ID() != 0) {
					String description = " Pago:" + payment.getDocumentNo();
					if (payment.getC_DocType().getDocBaseType().equals(MDocType.DOCBASETYPE_APPayment)) {
						fl = fact.createLine(line, line.getChargeAccount(as, line.getAmtSource()), getC_Currency_ID(),
								line.getAmtSource().negate(), null);
						if (fl != null && payment != null) {
							fl.setAD_Org_ID(payment.getAD_Org_ID());
							fl.setDescription(fl.getDescription() + description);
						}
						fl = fact.createLine(line, getPaymentAcct(as, line.getC_Payment_ID()), getC_Currency_ID(), null,
						...
						fl = fact.createLine(line, line.getChargeAccount(as, line.getAmtSource()), getC_Currency_ID(),
								null, line.getAmtSource());
						if (fl != null && payment != null) {
							fl.setAD_Org_ID(payment.getAD_Org_ID());
							fl.setDescription(fl.getDescription() + description);
						}
						fl = fact.createLine(line, getPaymentAcct(as, line.getC_Payment_ID()), getC_Currency_ID(),
			...
			// Sales Invoice
				...
				// SHW Vertrage
				if (invoice.get_ValueAsBoolean("isContract")) {
					for (MInvoiceLine ivl : invoice.getLines())
						acct_receivable = MCharge.getAccount(ivl.getC_Charge_ID(), as, line.getAmtSource());
				}

				if ((!as.isPostIfClearingEqual()) && acct_unallocated_cash != null
						&& acct_unallocated_cash.equals(acct_receivable) && (!isInterOrg)) {
					...
					if (line.getC_Payment_ID() != 0) {
						...
						fl = fact.createLine(line, getCashAcct(as, line.getC_CashLine_ID()), getC_Currency_ID(),
								line.getAmtSource(), null);
				...
				if (Env.ZERO.compareTo(line.getDiscountAmt()) != 0) {
					fl = fact.createLine(line, getAccount(Doc.ACCTTYPE_DiscountExp, as), getC_Currency_ID(),
				...
				if (Env.ZERO.compareTo(line.getWriteOffAmt()) != 0) {
					fl = fact.createLine(line, getAccount(Doc.ACCTTYPE_WriteOff, as), getC_Currency_ID(),
				...
				if (as.isAccrual()) {
					...
					flForRGL = factForRGL.createLine(line, bpAcct, getC_Currency_ID(), null, allocationSourceForRGL); // payment
																														// currency
					if (flForRGL != null)
			...
			// Purchase Invoice
				...
				if (invoice.get_ValueAsBoolean("isContract")) {
					for (MInvoiceLine ivl : invoice.getLines())
						acct_liability = MCharge.getAccount(ivl.getC_Charge_ID(), as, line.getAmtSource());
				}
				...
				if ((!as.isPostIfClearingEqual()) && acct_payment_select != null
						&& acct_payment_select.equals(acct_liability) && (!isInterOrg)) {
				...
				if (as.isAccrual()) {
					...
					// for Realized Gain & Loss
					flForRGL = factForRGL.createLine(line, bpAcct, getC_Currency_ID(), allocationSourceForRGL, null); // payment
																														// currency
					if (flForRGL != null)
				...
				// Discount CR
				if (Env.ZERO.compareTo(line.getDiscountAmt()) != 0) {
					fl = fact.createLine(line, getAccount(Doc.ACCTTYPE_DiscountRev, as), getC_Currency_ID(), null,
				...
				// Write off CR
				if (Env.ZERO.compareTo(line.getWriteOffAmt()) != 0) {
					fl = fact.createLine(line, getAccount(Doc.ACCTTYPE_WriteOff, as), getC_Currency_ID(), null,
				...
				{
					fl = fact.createLine(line, getCashAcct(as, line.getC_CashLine_ID()), getC_Currency_ID(), null,
							line.getAmtSource().negate());
					MCashLine cashLine = new MCashLine(getCtx(), line.getC_CashLine_ID(), getTrxName());
					if (fl != null && cashLine.get_ID() != 0)
						fl.setAD_Org_ID(cashLine.getAD_Org_ID());
			...
			// Realized Gain & Loss
			if (invoice != null && (getC_Currency_ID() != as.getC_Currency_ID() // payment allocation in foreign
																				// currency
					|| getC_Currency_ID() != line.getInvoiceC_Currency_ID())) // allocation <> invoice currency
			{
				p_Error = createRealizedGainLoss(line, as, fact, bpAcct, invoice, allocationSource,
						allocationAccounted);
				if (p_Error != null)
					return null;
			}
		...
		if ((!as.isPostIfClearingEqual()) && p_lines.length > 0 && (!isInterOrg)) {
			...
			// more than one line (i.e. crossing one payment+ with a payment-, or an invoice
	...
	private int getdefaultBank_Account_ID() {
		int defaultaccount = new Query(getCtx(), MBankAccount.Table_Name, "", null).setClient_ID()
				.setOnlyActiveRecords(true).setOrderBy("C_Bankaccount_ID").firstId();
	...
	private boolean isInterOrg(MAcctSchema as) {
		...
		for (int i = 0; i < p_lines.length; i++) {
			...
			if (line.getAD_Org_ID() != startorg || orgpayment != startorg || orginvoice != startorg
					|| orgcashline != startorg || orgorder != startorg)
				return true;
		}
	...
	private BigDecimal createCashBasedAcct(MAcctSchema as, Fact fact, MInvoice invoice, BigDecimal allocationSource) {
		/*
		 * BigDecimal allocationAccounted = Env.ZERO; // Multiplier double percent =
		 * invoice.getGrandTotal().doubleValue() / allocationSource.doubleValue(); if
		 * (percent > 0.99 && percent < 1.01) percent = 1.0; log.config("Multiplier=" +
		 * percent + " - GrandTotal=" + invoice.getGrandTotal() +
		 * " - Allocation Source=" + allocationSource);
		 * 
		 * // Get Invoice Postings Doc_Invoice docInvoice = (Doc_Invoice)Doc.get(new
		 * MAcctSchema[]{as}, MInvoice.Table_ID, invoice.getC_Invoice_ID(),
		 * getTrxName()); docInvoice.loadDocumentDetails(); allocationAccounted =
		 * docInvoice.createFactCash(as, fact, new BigDecimal(percent));
		 * log.config("Allocation Accounted=" + allocationAccounted);
		 * 
		 * // Cash Based Commitment Release if (as.isCreatePOCommitment() &&
		 * !invoice.isSOTrx()) { MInvoiceLine[] lines = invoice.getLines(); for (int i =
		 * 0; i < lines.length; i++) { Fact factC = Doc_Order.getCommitmentRelease(as,
		 * this, lines[i].getQtyInvoiced(), lines[i].getC_InvoiceLine_ID(), new
		 * BigDecimal(percent)); if (factC == null) return null; m_facts.add(factC);
		 * 
		 * } } // Commitment
		 * 
	...
	private MAccount getCashAcct(MAcctSchema as, int C_CashLine_ID) {
		String sql = "SELECT c.C_CashBook_ID " + "FROM C_Cash c, C_CashLine cl "
	...
	/**************************************************************************
	 * Create Realized Gain & Loss. Compares the Accounted Amount of the Invoice to
	 * the Accounted Amount of the Allocation
	 * 
	...
	private String createRealizedGainLoss(DocLine line, MAcctSchema acctSchema, Fact fact, MAccount account,
		...
		//
		String sql = "SELECT " + (invoice.isSOTrx() ? "SUM(AmtSourceDr), SUM(AmtAcctDr)" // so
		...
		if (getC_Currency_ID() != invoice.getC_Currency_ID()) {
			BigDecimal allocationSourceNew = MConversionRate.convert(getCtx(), allocationSource, getC_Currency_ID(),
		...
		MAccount gain = MAccount.getValidCombination(acctSchema.getCtx(),
				acctSchema.getAcctSchemaDefault().getRealizedGain_Acct(), getTrxName());
		MAccount loss = MAccount.getValidCombination(acctSchema.getCtx(),
				acctSchema.getAcctSchemaDefault().getRealizedLoss_Acct(), getTrxName());
		//
		if (invoice.isSOTrx()) {
			FactLine fl = fact.createLine(line, loss, gain, acctSchema.getC_Currency_ID(), acctDifference);
			fl.setDescription(description);
			fact.createLine(line, account, acctSchema.getC_Currency_ID(), acctDifference.negate());
			fl.setDescription(description);
		} else {
			fact.createLine(line, account, acctSchema.getC_Currency_ID(), acctDifference);
			FactLine fl = fact.createLine(line, loss, gain, acctSchema.getC_Currency_ID(), acctDifference.negate());
		}
		return null;
	} // createRealizedGainLoss

	/**************************************************************************
	 * Create Tax Correction. Requirement: Adjust the tax amount, if you did not
	 * receive the full amount of the invoice (payment discount, write-off). Applies
	 * to many countries with VAT. Example: Invoice: Net $100 + Tax1 $15 + Tax2 $5 =
	 * Total $120 Payment: $115 (i.e. $5 underpayment) Tax Adjustment = Tax1 = 0.63
	 * (15/120*5) Tax2 = 0.21 (5/120/5)
	 * 
	 * @param as              accounting schema
	 * @param fact            fact
	 ...
	 */
	private boolean createTaxCorrection(MAcctSchema as, Fact fact, DocLine_Allocation line, MAccount DiscountAccount,
			MAccount WriteOffAccoint, boolean isSOTrx) {
...
class Doc_AllocationTax {
	 ...
	 */
	public Doc_AllocationTax(MAccount DiscountAccount, BigDecimal DiscountAmt, MAccount WriteOffAccount,
			BigDecimal WriteOffAmt, boolean isSOTrx) {
