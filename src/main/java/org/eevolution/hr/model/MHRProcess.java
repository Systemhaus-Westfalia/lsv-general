	...
	*/
	...
	private Object executeScriptEngine(MHRConcept concept , MRule rule, String columnType)
	{
		...
		try {
			...
			// Set bindings directly on the engine (not a separate context)
		Bindings bindings = engine.createBindings();
			scriptCtx.entrySet().stream().forEach(entry ->
		...
		// Set the bindings on the engine
		engine.setBindings(bindings, ScriptContext.ENGINE_SCOPE);
		    result = rule.executeScript(engine);
