 ...
 */
public class SHW_EMailSender implements INotification {

	/**	Registration Id	*/
	private int registrationId = 0;
	/** Static Logger					*/
	private CLogger log = CLogger.getCLogger (SHW_EMailSender.class);
	...
	public void sendNotification(MADNotificationQueue notification) {
		...
		notification.getRecipients().forEach(recipient -> {
			...
			if (eMailConfigurationId <= 0 || eMailUser == null || eMailUser.isEmpty() || eMailPassword == null || eMailPassword.isEmpty()) {
				...
				//	Create instance
				SHW_EMail  email = new SHW_EMail(client,
				...
				recipient.setIsActive(false);
					...
					boolean isSent = SHW_EMail.SENT_OK.equals(email.send());
					if (isSent) {
