 ...
 */
public class shw_FlushSystemQueue extends shw_FlushSystemQueueAbstract

{
	...
	protected String doIt() throws Exception
	{
		if(getBatchsToProcess() > 0
			...
			if(invoiceID > 0) {
				whereClause.append("AND AD_Table_ID=318 AND ").append(X_AD_Queue.COLUMNNAME_Record_ID).append(" = ?");
				...
				IntStream.range(0, getBatchsToProcess()).forEach(page -> {
					...
					.forEach(queueId -> {
						...
						if(isValidToProcess(queueToProcess, referenceTime.get())) {
							...
							try {
								queueManager.process(queueToProcess, false);
