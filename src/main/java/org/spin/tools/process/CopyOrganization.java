...
/** Generated Process for (Copy Organization)
 *  @author ADempiere (generated) 
 *  @version Release 3.9.3
 */
public class CopyOrganization extends CopyOrganizationAbstract {
	...
	/**	Record Created	*/
	private Map<String, Integer> recordMap = new HashMap<String, Integer>();
	...
	protected String doIt() throws Exception {
		try {
			...
			Env.setContext(getCtx(), WINDOW_THIS_PROCESS, "AD_Org_ID", currentOrganization.getAD_Org_ID());
			Env.setContext(getCtx(), "#AD_Org_ID", currentOrganization.getAD_Org_ID());
	...
	private void copyOrganization() {
		...
		//	Info
		int currentCountryId = MRegion.get(getCtx(), getRegionId()).getC_Country_ID();
	...
	private int createAndGetDocumentTypeId(int currentDocumentTypeId) {
		...
		String newName = getName(currentDocumentType.getName());
		Integer newDocumentTypeId = recordMap.get(newName + "DT");
		...
		recordMap.put(newName + "DT", newDocumentTypeId);
		return newDocumentTypeId;
	}
	...
	private void copyDocumentTypeWithholding(int currentDocumentTypeId, int newDocumentTypeId) {
		//	For Role
		/*
		 * new Query(getCtx(), I_WH_DefinitionLine.Table_Name,
	...
	private int createAndGetSequenceId(int currentSequenceId) {
		...
		Integer newSequenceId = recordMap.get(newName + "SQ");
		...
		recordMap.put(newName + "SQ", newSequenceId);
		return newSequenceId;
	}
	...
	private int createAndGetPointOfSales(int currentPointOfSalesId) {
		...
		newPointOfSales.setM_Warehouse_ID(createAndGetWarehouse(newPointOfSales.getM_Warehouse_ID()));
	...
	private int createAndCashAccount(int currentCashAccountId) {
		...
		Integer newCashAccountId = recordMap.get(newName + "CA");
		...
		recordMap.put(newName + "CA", newCashAccountId);
		return newCashAccountId;
	}
	...
	private int createAndGetCashBook(int currentCashBookId) {
		...
		Integer newCashBookId = recordMap.get(newName + "CB");
		...
		recordMap.put(newName + "CB", newCashBookId);
		return newCashBookId;
	}
	...
	private int createAndGetWarehouse(int currentWarehouseId) {
		...
		Integer newWarehousetId = recordMap.get(newName + "WH");
		if(newWarehousetId != null) {
		...
		recordMap.put(newName + "WH", newWarehousetId);
		return newWarehousetId;
	}
